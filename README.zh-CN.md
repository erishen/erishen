...
- [分布式订单系统演进：Spring Boot + React 如何从单体到抗住高并发](https://erishen.cn/spring_order/) — Spring Boot + React 订单系统从单体到分布式、抗住高并发的演进路径。
...
- [cicdkit 工程实践：一个用纯 Go 标准库打造的本地 CI/CD 单二进制平台](https://erishen.cn/cicdkit/) — 只用 Go 标准库构建本地 CI/CD 平台，单二进制部署。
